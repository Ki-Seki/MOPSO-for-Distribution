...
% 本情景中输入的全排列是相当离散的数据，因此 repository 大小基本在 10 个以内，
% 所以其帕累托前沿从观感上，不像是一个曲线
...
rand_type = 'state';  % 随机数类型（无须动）
rand_seed = 1;  % 随机数种子（无须动）

dataset = 'c21';  % 数据集名称（本程序所解决的对象，文件必须存在，不包括文件后缀名）

loop_cnt = 150;  % 进化次数（任意正整数，越小越好）
particle_cnt = 200;  % 粒子数目（任意正整数，越小越好）
w = 1.5;  % 惯性权重（可以不动，值小于 5，越小越好）
c1 = 4;  % 自我学习因子（两个学习因子建议相等，值在 1 到 5 之间）
c2 = 4;  % 群体学习因子

graph_option.detail = false;  % 是否详细绘图（合法值：true，false）
% 由于绘图量比较大，所以预留 graph_option 为结构体，方便未来增加字段
